...
# enmuration for incidents
class Incident(enum.Enum):
    ReadyToOrder = "1"
    WantFood = "2"
    CheckOut = "3"
    CreditCard = "4"
    ...
    DropDrink = "6"
    FoodReplace = "7"
    # This two seems not been used for now
    HiddenRequest = "8"
    Idle = "9"
...
def gen_text_groups(customers, misc, sentences, gen_all=TRAVERSE):
    ...
    for c_key in customers:
        ...
        if Incident.ReadyToOrder.value in incidents and gen_all == False:
            ...
            # generate sentence
            name = c_key + incident_key[0] + "_" + " ".join(order) + ".wav"
            ...
            for st_key in sentences[incident_key[0]]:
                ...
                greeting = sentences[incident_key[0]].get("s00", "")
                name = c_key + incident_key[0] + st_key + "_" + " ".join(order) + ".wav"
...
def read_customer(customers_text):
    ...
    for sec in customers_ini:
        ...
        if "-Customer" in sec:
            ...
            c_idx = int(c.strip("Customer")) # customer index
            g = "g%d"%g_idx
            c = "c%d"%c_idx
            name = g + c
            customers[name] = {}
